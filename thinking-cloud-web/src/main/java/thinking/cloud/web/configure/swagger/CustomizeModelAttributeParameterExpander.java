package thinking.cloud.web.configure.swagger;
...
public class CustomizeModelAttributeParameterExpander extends ModelAttributeParameterExpander {
	...
	@Autowired
	public CustomizeModelAttributeParameterExpander(FieldProvider fields, AccessorsProvider accessors,
	...
	public List<Parameter> expand(ExpansionContext context) {
		...
		FluentIterable<ModelAttributeField> attributes = allModelAttributes(propertyLookupByGetter, getters,
				fieldsByName, alternateTypeProvider);

		FluentIterable<ModelAttributeField> expendables = attributes.filter(not(simpleType()))
		...
		return FluentIterable.from(parameters).filter(not(hiddenParameters())).filter(not(voidParameters())).toList();
	}

	private FluentIterable<ModelAttributeField> allModelAttributes(
			Map<Method, PropertyDescriptor> propertyLookupByGetter, Iterable<ResolvedMethod> getters,
			Map<String, ResolvedField> fieldsByName, AlternateTypeProvider alternateTypeProvider) {
		...
		FluentIterable<ModelAttributeField> modelAttributesFromFields = from(fieldsByName.values())
				.filter(publicFields()).transform(toModelAttributeField(alternateTypeProvider));
	...
	private Function<ResolvedField, ModelAttributeField> toModelAttributeField(
		...
		return new Function<ResolvedField, ModelAttributeField>() {
			...
			public ModelAttributeField apply(ResolvedField input) {
				return new ModelAttributeField(alternateTypeProvider.alternateFor(input.getType()), input.getName(),
						input, input);
			}
		};
	}
	...
	private Parameter simpleFields(String parentName, ExpansionContext context, ModelAttributeField each) {
		...
		ParameterExpansionContext parameterExpansionContext = new ParameterExpansionContext(dataTypeName, parentName,
				...
				new ModelAttributeParameterMetadataAccessor(each.annotatedElements(), each.getFieldType(),
						each.getName()),
				context.getDocumentationContext().getDocumentationType(), new ParameterBuilder());
	...
	private Function<ResolvedMethod, ModelAttributeField> toModelAttributeField(
			final Map<String, ResolvedField> fieldsByName, final Map<Method, PropertyDescriptor> propertyLookupByGetter,
			final AlternateTypeProvider alternateTypeProvider) {
		...
		try {
			...
			for (PropertyDescriptor descriptor : descriptors) {
				...
				if (field != null) {
					...
					IgnoreSwaggerParameter ignoreSwaggerParameter = field
							.getDeclaredAnnotation(IgnoreSwaggerParameter.class);
				...
				if (descriptor.getReadMethod() != null) {
					...
					IgnoreSwaggerParameter ignoreSwaggerParameter = readMethod.getDeclaredAnnotation(IgnoreSwaggerParameter.class);
					if (ignoreSwaggerParameter != null) {
						continue;
					}
	...
	private Map<Method, PropertyDescriptor> propertyDescriptorsByMethod(final Class<?> clazz,
		...
		return FluentIterable.from(propertyDescriptors).filter(new Predicate<PropertyDescriptor>() {
			...
			public boolean apply(PropertyDescriptor input) {
				return input.getReadMethod() != null && !clazz.isAssignableFrom(Collection.class)
